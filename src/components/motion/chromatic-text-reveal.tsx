...
export type ChromaticTextRevealProps = {
  ...
  /** Allows the active word to wrap within the available width. */
  wrap?: boolean;
  className?: string;
};
...
export function ChromaticTextReveal({
  ...
  return (
    <span
      ref={ref}
      className={cn(wrap ? "block max-w-full" : "inline-flex items-baseline", className)}
    >
      {prefix ? (
        <span
          className={
            wrap ? "block max-w-full whitespace-pre-wrap break-words" : "whitespace-nowrap"
          }
        >
          {prefix}
          {hasWords ? "\u00A0" : null}
      ...
      {hasWords ? (
        <span className={cn("relative inline-grid", wrap && "max-w-full")}>
          ...
          <motion.span
            ...
            className={cn(
              "absolute start-0 top-0 bg-clip-text text-transparent [background-image:var(--chromatic-gradient)] [contain:paint]",
              wrap ? "whitespace-pre-wrap break-words" : "whitespace-nowrap",
            )}
            style={
              {
