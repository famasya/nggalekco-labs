...
function Marker({ marker, radius, onClick, onHover }: MarkerProps) {
  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      ...
      {/* Place marker or circular image at the top */}
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            className={cn(
              "cursor-pointer overflow-hidden rounded-full bg-neutral-900 text-white shadow-lg transition-transform duration-200",
              marker.icon === "place" &&
                "grid place-items-center border border-white bg-white p-[2px] shadow-[0_0_0_1px_rgba(17,24,39,0.45),0_2px_6px_rgba(0,0,0,0.25)]",
              marker.icon !== "place" && hovered && "scale-125 shadow-xl ring-1 ring-white/50",
            )}
            style={{
              width: marker.icon === "place" ? "8px" : "8px",
              height: marker.icon === "place" ? "8px" : "8px",
              transform: marker.icon === "place" ? `scale(${hovered ? 0.16 : 0.1})` : undefined,
            }}
          ...
          >
            {marker.icon === "place" ? (
              <span className="size-full rounded-full bg-gray-900" aria-hidden="true" />
            ) : marker.src ? (
              <img
                src={marker.src}
