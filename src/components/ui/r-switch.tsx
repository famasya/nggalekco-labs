...
const spring = { type: "spring" as const, duration: 0.2, bounce: 0.2 };
const springFast = { type: "spring" as const, duration: 0.12, bounce: 0 };
const springSnap = { type: "spring" as const, duration: 0.2, bounce: 0.2 };
...
function getSizing(size: SwitchSize): SwitchSizing {
  ...
  return {
    ...base,
    thumbTravel: Math.max(0, base.trackW - base.thumbSize - base.thumbMargin * 2),
  };
}
...
function useSwitchFieldIds({
  ...
  return {
    ariaDescribedBy: joinAriaIds(ariaDescribedBy, description ? descriptionId : undefined),
    ariaLabelledBy: hasText ? joinAriaIds(ariaLabelledBy, textLabelId) : ariaLabelledBy,
    controlId,
    descriptionId,
    hasText,
    labelId,
...
function useSwitchThumbMotion(
  ...
  const scaleControlsRef = React.useRef<AnimationPlaybackControls[]>([]);
  const scaleGenerationRef = React.useRef(0);
  ...
  }, [displayChecked, prefersReducedMotion, thumbTravel, thumbX]);

  const stopScaleAnimations = React.useCallback(() => {
    scaleGenerationRef.current += 1;
    scaleControlsRef.current.forEach((control) => control.stop());
  ...
  const playToggleSquash = React.useCallback(() => {
    ...
    const pressX = animate(thumbScaleX, 1.15, springFast);
    const pressY = animate(thumbScaleY, 0.88, springFast);
    scaleControlsRef.current = [pressX, pressY];

    Promise.all([pressX, pressY]).then(() => {
      if (scaleGenerationRef.current !== generation) {
        return;
      }
...
function SwitchFieldText({
  ...
  return (
    <span className="flex min-w-0 flex-col gap-0.5">
      {label ? (
        <span className={cn("text-foreground", labelSizeClassName, labelClassName)} id={labelId}>
          {label}
          {required ? (
...
function SwitchThumbVisual({
  ...
  return (
    <motion.span
      aria-hidden
      className="pointer-events-none z-10 grid place-items-center rounded-full text-[10px] leading-none"
      style={{
  ...
  return (
    ...
    >
      ...
      <SwitchPrimitive.Thumb asChild>
        <SwitchThumbVisual motionState={motionState} sizing={sizing} thumbContent={thumbContent} />
